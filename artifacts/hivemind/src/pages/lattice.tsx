...
interface AgentPersona {
  name: string; emoji: string; specialty: string; bio: string;
  color: string; bgColor: string; borderColor: string; shapLabel: string;
}

const PERSONAS: Record<string, AgentPersona> = {
  hive_polymarket:      { name: "HIVEMIND",    emoji: "🐝", specialty: "Crowd Intelligence",   bio: "Aggregates real-money Polymarket signals. Follows smart money with skin in the game.",                                            color: "text-purple-400",  bgColor: "bg-purple-500/10",  borderColor: "border-purple-500/25",  shapLabel: "Crowd Signal" },
  hypothesis_momentum:  { name: "MOMO",        emoji: "📈", specialty: "Trend Following",       bio: "Chases price action. Believes trends persist until they don't. RSI, MACD, MA crossover devotee.",                                color: "text-blue-400",    bgColor: "bg-blue-500/10",    borderColor: "border-blue-500/25",    shapLabel: "AI Technical" },
  hypothesis_meanrevert:{ name: "REVERTER",    emoji: "🔄", specialty: "Mean Reversion",        bio: "Fades extremes. Every 2σ deviation is an opportunity. Bollinger Bands specialist.",                                             color: "text-cyan-400",    bgColor: "bg-cyan-500/10",    borderColor: "border-cyan-500/25",    shapLabel: "AI Technical" },
  hypothesis_volregime: { name: "VOLT",        emoji: "⚡", specialty: "Vol-Regime Analysis",   bio: "Reads the macro environment. Calibrates all signals against calm / volatile / crisis regimes.",                                 color: "text-indigo-400",  bgColor: "bg-indigo-500/10",  borderColor: "border-indigo-500/25",  shapLabel: "AI Technical" },
  hypothesis_hive:      { name: "HIVE WISDOM", emoji: "🦋", specialty: "Crowd Synthesis",       bio: "Translates Polymarket crowd intelligence into a directional bias. Liquidity-weighted oracle.",                                   color: "text-violet-400",  bgColor: "bg-violet-500/10",  borderColor: "border-violet-500/25",  shapLabel: "Crowd Signal" },
  critique_devil:       { name: "SKEPTIC",     emoji: "😈", specialty: "Devil's Advocate",      bio: "Challenges consensus. Applies overconfidence penalties. The contrarian in the room.",                                           color: "text-orange-400",  bgColor: "bg-orange-500/10",  borderColor: "border-orange-500/25",  shapLabel: "AI Critique" },
  critique_tailrisk:    { name: "BLACKBIRD",   emoji: "🦅", specialty: "Tail Risk & Geopolitics", bio: "Models fat tails. Watches Hormuz, Taiwan, crypto contagion. Kurtosis is always underestimated.",                              color: "text-red-400",     bgColor: "bg-red-500/10",     borderColor: "border-red-500/25",     shapLabel: "Geo Risk" },
  synthesis:            { name: "ORACLE",      emoji: "⚖️", specialty: "Bayesian Aggregation",  bio: "Reputation-weighted fusion of all agents. Platt-calibrated probability output.",                                               color: "text-primary",     bgColor: "bg-primary/10",     borderColor: "border-primary/25",     shapLabel: "Synthesis" },
  meta:                 { name: "ARBITER",     emoji: "🧠", specialty: "Final Verdict",         bio: "Regime-calibrated final call. Issues the Hivemind Score. The last word.",                                                      color: "text-amber-400",   bgColor: "bg-amber-500/10",   borderColor: "border-amber-500/25",   shapLabel: "Meta" },
};

// ─── Challenge Hook ───────────────────────────────────────────────────────────
interface ChallengeResult { response: string; adjustment: number; newProbability: number; }
...
function useChallengeAgent(symbol: string) {
  ...
  const challenge = async (agentType: string, text: string, prob: number) => {
    ...
    try {
      ...
      setResults(s => ({ ...s, [agentType]: { loading: false, response: "Challenge failed — API unreachable.", adjustment: 0, newProbability: prob } }));
    }
  };
...
function buildConvergenceData(tokens: BeliefToken[]) {
  const ROUNDS = [{ round: 0, label: "Hive" }, { round: 1, label: "Hypothesis" }, { round: 2, label: "Critique" }, { round: 3, label: "Synthesis" }, { round: 4, label: "Final" }];
  return ROUNDS.map(({ round, label }) => {
    ...
    rt.forEach(t => { entry[t.agentType] = parseFloat(t.probability.toFixed(3)); });
    ...
    <div className={`rounded-xl border ${p.borderColor} ${p.bgColor} p-3 flex flex-col gap-2`}>
      ...
      <div className="flex items-center gap-2 mt-auto">
        <div className="flex-1 h-1 bg-white/[0.05] rounded-full overflow-hidden">
          <div className={`h-full rounded-full ${p.bgColor.replace("/10", "")} opacity-80`} style={{ width: `${Math.min(100, reputation * 80)}%` }} />
        </div>
...
function DebateAgentCard({ token, symbol, upvotes, onUpvote, challengeResult, onChallenge }: {
  token: BeliefToken; symbol: string; upvotes: number; onUpvote: () => void;
  ...
  return (
    <div className={`rounded-xl border ${p.borderColor} bg-black/20 overflow-hidden`}>
      <div className="p-3.5 flex items-start gap-3 cursor-pointer select-none" onClick={() => setExpanded(o => !o)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[9px] font-mono text-muted-foreground shrink-0">CONF {(token.confidence * 100).toFixed(0)}%</span>
          </div>
      ...
      <div className="px-3.5 pb-1">
        ...
        {token.rationale.length > 2 && (
          <button className="text-[10px] font-mono text-muted-foreground/50 hover:text-muted-foreground mt-1.5 flex items-center gap-1" onClick={() => setExpanded(o => !o)}>
      ...
      {challengeResult?.response && (
        <div className="mx-3.5 mb-2 mt-1 bg-black/30 rounded-lg p-3 border border-white/[0.06]">
          <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest mb-2">{p.name} responds:</div>
        ...
        <div className="mx-3.5 mb-2 mt-1">
          <div className="flex gap-2 items-center bg-black/30 border border-white/10 rounded-lg px-3 py-2">
            <input
              ...
              autoFocus
            />
            <button onClick={submitChallenge} disabled={!challengeText.trim() || challengeResult?.loading} className="text-primary hover:text-white disabled:opacity-40 transition-colors">
              {challengeResult?.loading ? <div className="w-4 h-4 border border-current border-t-transparent rounded-full animate-spin" /> : <Send className="w-4 h-4" />}
            </button>
          </div>
          <p className="text-[9px] text-muted-foreground/50 font-mono mt-1 ml-1">Inject new information and see how the agent revises its probability.</p>
        </div>
      )}
      ...
      <div className="px-3.5 py-2.5 border-t border-white/[0.05] flex items-center gap-3">
        ...
        <div className="ml-auto text-[9px] font-mono text-muted-foreground/40">R{token.round} · {token.id.slice(0, 6)}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <svg width="88" height="88" viewBox="0 0 96 96">
        ...
        <circle cx="48" cy="48" r="36" fill="none" stroke={color} strokeWidth="7" strokeDasharray={`${dash} ${circ - dash}`} strokeLinecap="round" transform="rotate(-90 48 48)" style={{ filter: `drop-shadow(0 0 6px ${color}80)` }} />
        <text x="48" y="53" textAnchor="middle" fontSize="18" fontWeight="700" fill="white" fontFamily="JetBrains Mono">{score.toFixed(0)}</text>
      </svg>
...
function DivergenceIndicator({ tokens }: { tokens: BeliefToken[] }) {
  const debateTokens = tokens.filter(t => ["hypothesis_momentum","hypothesis_meanrevert","hypothesis_volregime","hypothesis_hive","critique_devil","critique_tailrisk"].includes(t.agentType));
  ...
  const label = isStrong ? `STRONG ${bullish >= 4 ? "BULL" : "BEAR"} CONSENSUS` : isSplit ? "AGENTS SPLIT" : "MIXED SIGNALS";
  const labelColor = isStrong ? (bullish >= 4 ? "text-emerald-400" : "text-red-400") : "text-amber-400";
  const borderColor = isStrong ? (bullish >= 4 ? "border-emerald-500/20" : "border-red-500/20") : "border-amber-500/20";
  const bgColor = isStrong ? (bullish >= 4 ? "bg-emerald-500/5" : "bg-red-500/5") : "bg-amber-500/5";
  return (
    <div className={`rounded-xl border ${borderColor} ${bgColor} p-3.5 flex items-center gap-4`}>
      ...
      <div className="flex items-center gap-2 ml-auto">
        <div className="flex gap-0.5">
          {debateTokens.map(t => (
            <div key={t.id} className={`w-3 h-3 rounded-sm ${t.probability > 0.54 ? "bg-emerald-500" : t.probability < 0.46 ? "bg-red-500" : "bg-amber-400"}`} title={`${PERSONAS[t.agentType]?.name ?? t.agentType}: ${(t.probability * 100).toFixed(0)}%`} />
          ))}
        </div>
        <span className="text-[10px] font-mono text-muted-foreground">{bullish}B · {bearish}S · {neutral}N</span>
        <span className={`text-[10px] font-mono font-600 ${labelColor}`}>{(consensusPct * 100).toFixed(0)}% agree</span>
      </div>
    </div>
  );
}
...
function ConvergenceChart({ tokens }: { tokens: BeliefToken[] }) {
  ...
  const agentColors: Record<string, string> = {
    hive_polymarket: "#a855f7", hypothesis_momentum: "#60a5fa", hypothesis_meanrevert: "#22d3ee",
    hypothesis_volregime: "#818cf8", hypothesis_hive: "#8b5cf6", critique_devil: "#f97316",
    critique_tailrisk: "#f87171", synthesis: "#00d4ff", meta: "#fbbf24",
  };
  return (
    <Card className="bg-card/60 border-white/[0.07] backdrop-blur-sm">
      <CardContent className="p-4">
        <div className="data-label mb-1">Belief Convergence</div>
        <p className="text-[10px] text-muted-foreground/70 font-mono mb-3">Probability evolution across debate rounds — bold = consensus</p>
        <div className="h-44">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" vertical={false} />
              <XAxis dataKey="label" stroke="#ffffff30" fontSize={9} fontFamily="JetBrains Mono" />
              <YAxis domain={[0, 1]} stroke="#ffffff30" fontSize={9} fontFamily="JetBrains Mono" tickFormatter={v => `${(v * 100).toFixed(0)}%`} width={36} />
              <Tooltip contentStyle={{ background: "hsl(222,32%,8%)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, fontFamily: "JetBrains Mono", fontSize: 11 }} formatter={(v: number, name: string) => [`${(v * 100).toFixed(1)}%`, PERSONAS[name]?.name ?? name]} labelStyle={{ color: "#ffffff80", marginBottom: 4 }} />
              <ReferenceLine y={0.5} stroke="rgba(255,255,255,0.12)" strokeDasharray="4 4" />
              {agentTypes.map(type => (
                <Line key={type} type="monotone" dataKey={type} stroke={agentColors[type] ?? "#ffffff30"} strokeWidth={1} strokeOpacity={0.3} dot={false} connectNulls isAnimationActive={false} />
              ))}
              <Line type="monotone" dataKey="consensus" stroke="#00d4ff" strokeWidth={2.5} dot={{ fill: "#00d4ff", strokeWidth: 0, r: 3 }} connectNulls isAnimationActive style={{ filter: "drop-shadow(0 0 6px rgba(0,212,255,0.5))" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
    ...
    <Card className="bg-card/60 border-white/[0.07] backdrop-blur-sm overflow-hidden">
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[9px] font-mono text-muted-foreground ml-1">feeding BLACKBIRD + HIVEMIND agents</span>
        </div>
        <div className="space-y-2.5">
          {visible.map((item, i) => {
            ...
            return (
              <div key={i} className={`rounded-lg border p-3 ${isHigh ? "border-orange-500/20 bg-orange-500/5" : "border-white/[0.06] bg-white/[0.02]"}`}>
                <p className="text-[11px] text-white/90 font-semibold leading-snug mb-1.5">{item.headline}</p>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5 mb-1">
                      <div className="flex-1 h-1 bg-white/[0.05] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${isHigh ? "bg-orange-500/60" : "bg-amber-500/40"}`} style={{ width: `${item.yesPrice * 100}%` }} />
                      </div>
                    </div>
                    <p className="text-[9px] font-mono text-muted-foreground/60">{item.marketImpact}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className={`text-[16px] font-mono font-700 ${isHigh ? "text-orange-400" : "text-amber-400"}`}>{pct}%</div>
        ...
        {intel.length > 3 && (
          <button onClick={() => setExpanded(o => !o)} className="w-full text-[10px] font-mono text-muted-foreground/60 hover:text-white py-2 mt-1 transition-colors flex items-center justify-center gap-1">
            {expanded ? <><ChevronUp className="w-3 h-3" /> Show less</> : <><ChevronDown className="w-3 h-3" /> {intel.length - 3} more signals</>}
          </button>
        )}
      </CardContent>
    </Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono font-700 text-white uppercase tracking-widest">Self-Improvement Cycle</span>
            </div>
          ...
          >
            {train.isPending ? (
              <><div className="w-3 h-3 border border-current border-t-transparent rounded-full animate-spin" />Training…</>
            ) : (
              <><RefreshCw className="w-3 h-3" />Run Training</>
            )}
          </Button>
          ...
          <div className="space-y-2.5 pt-1">
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { label: "Accuracy Δ", value: `${result.accuracyGain >= 0 ? "+" : ""}${(result.accuracyGain * 100).toFixed(2)}%`, color: result.accuracyGain >= 0 ? "text-emerald-400" : "text-red-400" },
              ].map(stat => (
                <div key={stat.label} className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-2.5 text-center">
            ...
            <p className="text-[10px] font-mono text-muted-foreground/80 leading-relaxed">{result.message}</p>

            {result.agentUpdates.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">Agent Reputation Updates</div>
                {result.agentUpdates.map(u => {
                  const p = PERSONAS[u.agentType];
                  return (
                    <div key={u.agentType} className="flex items-center gap-2">
                      <span className="text-base shrink-0">{p?.emoji ?? "🤖"}</span>
                      <span className={`text-[10px] font-mono flex-1 ${p?.color ?? "text-white"}`}>{p?.name ?? u.agentType}</span>
                      <span className="text-[9px] font-mono text-muted-foreground">{u.oldReputation.toFixed(2)} →</span>
                      <span className={`text-[10px] font-mono font-600 ${u.delta > 0 ? "text-emerald-400" : u.delta < 0 ? "text-red-400" : "text-muted-foreground"}`}>{u.newReputation.toFixed(2)}</span>
                      <span className={`text-[9px] font-mono ${u.delta > 0 ? "text-emerald-400/70" : u.delta < 0 ? "text-red-400/70" : "text-muted-foreground/40"}`}>
                        {u.delta > 0 ? "↑" : u.delta < 0 ? "↓" : "="}
                      </span>
                    </div>
                  );
...
// ─── Debate Phases ────────────────────────────────────────────────────────────
function DebatePhase({ title, subtitle, tokens, symbol, upvotes, onUpvote, challenges, onChallenge }: {
  title: string; subtitle: string; tokens: BeliefToken[]; symbol: string;
  upvotes: Record<string, number>; onUpvote: (type: string) => void;
  ...
  return (
    <div>
      ...
      <div className="space-y-3 pl-2 border-l border-white/[0.05] ml-2">
        {tokens.map(token => (
          <DebateAgentCard
            key={token.id} token={token} symbol={symbol}
            upvotes={upvotes[token.agentType] ?? 0} onUpvote={() => onUpvote(token.agentType)}
...
function DebateView({ result, upvotes, onUpvote }: { result: LatticeResult; upvotes: Record<string, number>; onUpvote: (type: string) => void }) {
  const { results: challenges, challenge } = useChallengeAgent(result.symbol);
  const phases = [
    { title: "Phase 1 · Crowd Signal",        subtitle: "Polymarket real-money consensus",                        types: ["hive_polymarket"] },
    { title: "Phase 2 · Hypothesis Formation", subtitle: "4 independent agents present their directional thesis",  types: ["hypothesis_momentum","hypothesis_meanrevert","hypothesis_volregime","hypothesis_hive"] },
    { title: "Phase 3 · Critique Round",       subtitle: "Devil's advocate and tail-risk specialists challenge",   types: ["critique_devil","critique_tailrisk"] },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {phases.map(phase => (
          <DebatePhase
            key={phase.title} title={phase.title} subtitle={phase.subtitle}
            tokens={debateTokens.filter(t => phase.types.includes(t.agentType))}
            symbol={result.symbol} upvotes={upvotes} onUpvote={onUpvote}
            challenges={challenges} onChallenge={challenge}
          />
        ))}
      </div>
        ...
        <div>
          ...
          <div className="space-y-3 pl-2 border-l border-white/[0.05] ml-2">
            {[synthToken, metaToken].filter(Boolean).map(token => (
              <DebateAgentCard key={token!.id} token={token!} symbol={result.symbol} upvotes={0} onUpvote={() => {}} challengeResult={undefined} onChallenge={() => {}} />
            ))}
          </div>
        </div>
      )}
          ...
          <div className="space-y-1.5">
            {result.causalNarrative.split("\n").filter(l => l.trim()).map((line, i) => (
              <p key={i} className={`text-[11px] leading-relaxed font-mono ${
                ...
                : line.startsWith("Market Regime") || line.startsWith("Signal") || line.startsWith("Price Target") ? "text-white/80 font-medium"
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="flex gap-2">
        <TickerCombobox
          value={symbol} onChange={sym => setSymbol(sym)}
        ...
        <Select value={timeframe} onValueChange={setTimeframe}>
          ...
          <SelectContent>
            {TIMEFRAMES.map(t => <SelectItem key={t} value={t} className="font-mono">{t.toUpperCase()}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      {!result && (
        <>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {DEBATE_AGENTS.map(type => (
                <AgentRosterCard key={type} agentType={type} reputation={agentList.find(a => a.agentType === type)?.reputation ?? 1.0} />
              ))}
            </div>
          </div>
          <div className="bg-black/20 border border-white/[0.06] rounded-xl p-3.5 text-[11px] font-mono text-muted-foreground leading-relaxed">
            ...
            Hive reads live Polymarket crowd signals → 4 hypothesis agents form directional theses → 2 critique agents challenge the consensus → Bayesian synthesis → Meta-agent final verdict. Challenge any agent mid-debate to inject new information. Run training cycles to improve agent accuracy over time.
          </div>
        </>
      )}

      {/* Run button */}
      <Button className="w-full gap-2 h-11 font-mono" onClick={handleRun} disabled={!symbol || runLattice.isPending}>
        {runLattice.isPending ? (
          <><div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />Running 6 agents · 4 debate rounds + geo intel…</>
        ) : (
          <><Zap className="w-4 h-4" />{result ? `Re-run HPL Lattice${symbol ? ` · ${symbol}` : ""}` : `Activate Lattice${symbol ? ` · ${symbol}` : ""}`}</>
        )}
      </Button>
                  ...
                  <div key={agent.agentId} className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="w-16 h-1 bg-white/[0.05] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${p.bgColor}`} style={{ width: `${Math.min(100, agent.reputation * 80)}%` }} />
                      </div>
                      <span className="text-[11px] font-mono text-white w-8 text-right">{agent.reputation.toFixed(2)}</span>
                      <span className="text-[10px] font-mono text-muted-foreground w-10 text-right">{agent.totalRuns > 0 ? `${agent.correctRuns}/${agent.totalRuns}` : "—"}</span>
                    </div>
                  </div>
                );
              })}
