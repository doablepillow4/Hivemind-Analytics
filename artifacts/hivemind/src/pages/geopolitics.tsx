    ...
    <Card className="bg-card/60 border-white/[0.07] backdrop-blur-sm overflow-hidden">
      ...
      <CardContent className="p-4">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {visibleWithMarkets.map(({ item, relatedMarkets }, i) => (
              <NewsIntelCard key={item.id ?? i} item={item} relatedMarkets={relatedMarkets} />
            ))}
            {items.length > 6 && (
              <button
    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-0.5 -mx-1 px-1 no-scrollbar">
        {Object.entries(REGION_CONFIG).map(([key, cfg]) => (
          <button
            ...
            className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-mono font-600 transition-all ${
              activeRegion === key
                ? "bg-primary/15 text-primary border border-primary/30"
                : "bg-white/[0.04] text-muted-foreground border border-white/[0.07] hover:border-white/15 hover:text-white"
            }`}
          >
            <span>{cfg.flag}</span>
            <span>{cfg.label}</span>
      ...
      {marketsLoading ? (
        ...
        <div className="text-center py-12 border border-dashed border-white/10 rounded-xl">
          <Globe className="w-7 h-7 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-[12px] text-muted-foreground">No markets match this filter</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((market) => (
